...
async function fetchTimeout(url,opts={},timeout=25000,label='HTTP'){
  let last;
  for(let n=0;n<4;n++){
    const attempt=n+1;
    try{
      console.log(`[${label}] attempt ${attempt}/4 ${url}`);
...
async function openskyGet(url,timeout=25000){let headers=await authHeaders();let opts={headers};const user=clean(process.env.OPENSKY_USERNAME), pass=clean(process.env.OPENSKY_PASSWORD);if(!headers.Authorization&&user&&pass)opts.headers={...UA,Authorization:'Basic '+Buffer.from(`${user}:${pass}`).toString('base64')};let r=await fetchTimeout(url,opts,timeout,'OpenSky');if(r.status===401&&clean(process.env.OPENSKY_CLIENT_ID)&&clean(process.env.OPENSKY_CLIENT_SECRET)){token=null;if(await fetchToken())r=await fetchTimeout(url,{headers:await authHeaders()},timeout,'OpenSky')}return r}
...
app.get('/api/health',(q,s)=>s.json({ok:true,name:'DispatchLink iPad',version:'1.3.1',generator:'OpenSky+HexDB'}));
...
app.listen(process.env.PORT||3000,'0.0.0.0',()=>console.log('DispatchLink iPad 1.3.1 ready — OpenSky+HexDB desktop generator port'));
